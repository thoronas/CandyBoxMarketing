<?php
/*
Template Name: web-design
*/
?>

<?php
get_header();
?>
      <section id="content" class="clearfix">
			<div class="wrapper">
        <div id="webDesignPage" class="clearfix">
          <article class="works-box">
            <img src="<?php bloginfo('stylesheet_directory')?>/content/works.png" alt="" />
          </article>
          <!--END works-box-->
          <article class="works-cont clearfix">
            <header>
              <h1>Design for your <span>customers,</span> not <span>yourself</span></h1>
            </header>
            <div class="works-txt">
              <p class="descTxt"> Every good website design starts with your end-user in mind. </p>
              <p>
                <em>Don't settle for templates</em> or an online brochure. Your customer is worth more than a well written about us page. With our help, here is what you can expect: </p>
              <ul>
                <li>
                  <a href="#" title="">Full project management to start and end efficiently</a>
                </li>
                <li>
                  <a href="#" title="">A flow that targets how customers look for information</a>
                </li>
                <li>
                  <a href="#" title=""><em>Lead funnels</em> that walk prospects through the site</a>
                </li>
                <li>
                  <a href="#" title="">A Web-focused writing style: short, concise statements</a>
                </li>
                <li>
                  <a href="#" title="">Optimization for search engines (like Google) </a>
                </li>
                <li>
                  <a href="#" title=""><em>Social media</em> integrations</a>
                </li>
                <li>
                  <a href="#" title="">A very sweet visual style</a>
                </li>
              </ul>
              <div class="btnRow">
                <a class="brownBtn" href="#"><span>Ask us to help</span></a>
                <div class="blkLeftRow">
                  <a class="blackLink" href="#">See our work</a>
                </div>
              </div>
            </div>
            <!--END works-txt-->
            <div class="works-img"><img src="<?php bloginfo('stylesheet_directory')?>/content/works-1.png" alt="" /></div>
            <!--END works-img-->
          </article>
          <!--END works-cont-->
        </div>
        <!--END webDesignPage-->
        <footer class="bootBlk">
          <ul>
            <li><iframe src="http://www.facebook.com/plugins/like.php?href=http%3A%2F%2Fcandyboxmarketing.com%2F&amp;layout=button_count&amp;show_faces=true&amp;width=100&amp;action=like&amp;font=trebuchet+ms&amp;colorscheme=light&amp;height=21" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width: 100px; height:21px;"></iframe></li>
            <li><script type="text/javascript" src="http://platform.linkedin.com/in.js"></script><script type="in/share" data-counter="right"></script></li>
            <li>
              <a href="http://twitter.com/share" class="twitter-share-button" data-count="horizontal" data-via="candyboxmktg">Tweet</a>
              <script type="text/javascript" src="http://platform.twitter.com/widgets.js"></script>
            </li>
          </ul>
        </footer>
			</div>	
      </section>
      <!--END content-->
</div>
<!--END page-->

<?php 
get_footer();
?>
